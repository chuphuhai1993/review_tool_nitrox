...
async function scrapeReviewsFromCountry(appId, country, limit = 100) {
  try {
    ...
    // Crawl reviews từng batch
    while (reviews.length < limit) {
      const result = await gplay.reviews({
        appId: appId,
        sort: gplay.sort.NEWEST,
        num: Math.min(150, limit - reviews.length),
        nextPaginationToken: nextPaginationToken,
        country: country, // Chỉ định country
      });
      ...
      const reviewsWithCountry = result.data.map(review => ({
        ...review,
        country: country
      }));
...
async function main() {
  ...
  console.log('   GOOGLE PLAY REVIEWS SCRAPER');
  console.log('='.repeat(60));
